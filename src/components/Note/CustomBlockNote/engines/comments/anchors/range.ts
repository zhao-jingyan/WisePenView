...
const RANGE_COMMENT_YJS_ORIGIN = 'wisePenRangeCommentSync';
...
function orphanCommentMarksForThreadIds(
  editor: CustomBlockNoteEditor,
...
function applyCommentMarkToRange(
  editor: CustomBlockNoteEditor,
...
export function syncPlainTextCommentDocumentMarks(
  ...
  const hiddenThreadIds = visibilityScope
    ? getHiddenThreadIdsForUser(Array.from(threadsYMap.values()) as ThreadData[], visibilityScope)
    : new Set<string>();
