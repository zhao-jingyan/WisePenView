import { useMessageScroller, useMessageScrollerScrollable } from '@/components/_shadcn';
...
function ReasoningBlock({ content, loading }: ReasoningBlockProps) {
  ...
  const wasFollowingBottomRef = useRef(!canScrollToEnd);
  const open = loading || !collapsed;

  /**
   * 推理流结束会同步更新区块状态并开始渲染后续正文，二者在同一批次时可能错过 scroller 的尺寸观察。
   * 仅在切换前仍跟随底部时，于提交后的下一帧补一次下滚；cleanup 用于避免组件卸载后执行滚动。
   */
  useEffectForce(() => {
    ...
    if (!completed) {
      wasFollowingBottomRef.current = !canScrollToEnd;
      return;
    }

    if (!wasFollowingBottomRef.current) return;
  ...
  }, [canScrollToEnd, loading, scrollToEnd]);
