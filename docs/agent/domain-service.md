...
Service 层位于 `src/domains/<Domain>/service`，是 view/component 获取领域能力的主要入口。它负责业务编排，不负责修补协议字段。
...
- 跨领域复用逻辑应提升到 mapper、明确命名的 normalizer、`src/utils` 或 `src/domains/_shared`。
...
- 纯数据转换不要在 service 中展开，交给 mapper 或领域 normalizer。
...
- API response 字段别名、历史兼容、空值归一化放到 mapper。
- 请求参数默认值可以存在，但要表达清楚业务含义，例如分页默认页码。
- Service 不向组件透传需要二次兜底的半成品数据；组件需要直接展示的字段应在 mapper/entity 中补齐。
