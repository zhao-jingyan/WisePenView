...
`docs/agent` 是 WisePenView 面向 agent 的专题规约目录。根目录 `AGENT.md` 是唯一总则；本目录只补充分层、目录和局部约定。

不要一次性预读所有文件，按任务类型选择需要的专题文档。若专题文档与 `AGENT.md` 冲突，以 `AGENT.md` 为准。
...
新增代码遵循 `AGENT.md` 描述的最终形态；修改旧代码时，只收敛与当前任务相关的部分，不做无关大重构。
