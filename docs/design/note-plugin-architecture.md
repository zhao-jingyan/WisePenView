...
> 状态：已实施第一阶段，2026-07-13。
>
> 第 2 节记录迁移前快照；第 4-8 节描述当前已落地契约；第 9-11 节保留实施过程和删除记录。Note session 生命周期、Yjs 数据结构和同步协议未进入本轮修改。

已落地：

- 树形 plugin registry、唯一 block/inline owner、依赖/冲突/能力实现校验；
- schema/render、projection、Markdown import/export、下载 artifact、AI Diff、print、comments selection、side menu 的 owner 分发；
- `AiChangePlugin` 只拥有五种 AI 语法 inline content，协议、遍历和批量操作收敛到 `runtime/aiDiff`；
- 页面只执行 Markdown 下载副作用，浏览器 print engine 只聚合 owner 样式；
- 编辑器入口 props 已按 collaboration/state/comments/portals 收口，页面批注 actor 复用；
- 56 个 Note 专项测试，覆盖 registry、Markdown、projection、AI Diff、comments policy、side menu 和新笔记状态；
- 删除旧插件 API、重复 visitor、无消费者 selection store、旧 AI export path 和 785 行未接入 mock。

尚未完成：`CustomBlockNote` 仍编排图片上传、comments data source 和公式 comments surface；projection 仍通过多个 callback 暴露。这些是 Runtime/页面边界的下一阶段，不影响内容 owner 架构成立。

## 1. 背景与目标

迁移前 `CustomBlockNote` 同时承担编辑器创建、内容能力、批注编排、AI Diff、Markdown 导入导出、PDF 打印、目录投影、页面通信和业务 service 调用。旧插件是扁平技术能力集合，不是内容类型的所有权边界。
...
迁移前 `src/components/Note/CustomBlockNote/index.tsx` 同时处理：
...
迁移前 `src/components/Note/CustomBlockNote/plugins/types.ts` 的插件契约只有：
...
- `DefaultContentPlugin/aiDiff.ts` 拥有 text、link、富文本 block 和无正文原子 block 的语义；
- `LatexPlugin/aiDiff.ts` 拥有 MathBlock、InlineMath 的协议转换、presence、accept/discard 和 Markdown projection；
- `AiChangePlugin` 拥有 `ai-diff`、`ai-add`、`ai-delete`、`ai-link-add`、`ai-link-delete` 五种 inline content；
- `runtime/aiDiff` 只负责 registry dispatch、协议递归、generated block 规范化、display mode、批量操作和编辑器 extension。

后端 `AI-content` 协议也通过 owner adapter 转换。Inline owner 提供 `protocol` 的 normalize、visibleText、plain/create/delete/edit；block owner 提供 `normalizeProtocol`。Runtime 不再识别 text、link、inlineMath、math 或 AI syntax type。Yjs adapter 因本轮不修改同步协议，保留既有 text/link 序列化逻辑并隔离在 `runtime/aiDiff/yjs`。
...
- `inherited`：owner 直接复用共享 adapter，registry 仍校验实现存在；
...
| codeBlock        | custom                    | default                    | default            | unsupported，当前映射明确排除       | range text selection            | custom toolbar/transform/slash   | 通用 inline plain text              |
| table            | default + custom handles  | default lossy              | default lossy      | unsupported                         | unsupported，selection 尚未建模 | custom toolbar/side menu/handles | owner 返回空 plain text             |
...
MathBlock owner 使用结构化 `renderMarkdown` 输出独立 `$$` 围栏，避免 BlockNote lossy exporter 把多行 expression 压平。导入器同时识别独立围栏和历史实际输出 `$$ expression $$`；通用 exporter 只做 owner dispatch 和默认 codec 分段，不做整篇字符串后处理。

## 8. 测试策略

仓库已引入最小 Vitest 基础设施，测试文件与被测模块同域放置；当前 Note 专项共有 56 个测试，不依赖 dev/mock 服务。
...
`AI-Create`、`AI-Delete`、`AI-Edit` 是后端 `AI-content` 转换过程的活跃中间协议类型，不是当前 schema 注册的 inline type。它们只保留在 `runtime/aiDiff/protocol` 与 generated normalization 边界；旧 export/visibility 兼容路径已删除。
...
2. audio/file/video 是否继续作为只读旧文档兼容类型保留在 schema；当前 slash 隐藏，编辑器上传口只接受 image。
3. code/table 普通批注是否属于正式支持范围；当前 policy 为 code `range`、table `unsupported`，需要产品确认后再改变。

在这些问题有答案前，保持现有文档 schema 和协议，不扩大能力。CodeBlock、table、media 不承担 AI Diff，继续按 unsupported 建模。
