...
- 45 个 Note 专项测试，覆盖 registry、Markdown、projection、AI Diff、comments policy、side menu 和新笔记状态；
- 删除旧插件 API、重复 visitor、无消费者 selection store、旧 AI export path 和 785 行未接入 mock。

尚未完成：`CustomBlockNote` 仍编排图片上传、comments data source、公式 comments surface 和 AI bulk UI；projection 仍通过多个 callback 暴露。这些是 Runtime/页面边界的下一阶段，不影响内容 owner 架构成立。
...
仓库已引入最小 Vitest 基础设施，测试文件与被测模块同域放置；当前 Note 专项共有 45 个测试，不依赖 dev/mock 服务。
...
`AI-Create`、`AI-Delete`、`AI-Edit` 是后端 `AI-content` 转换过程的活跃中间协议类型，不是当前 schema 注册的 inline type。它们只保留在 proto/normalization 边界；旧 export/visibility 兼容路径已删除。
